...
| 4. LLM answer constraints | `providers/llm.py` | System prompt (Anthropic path) explicitly forbids diagnosis/prescription/dose changes and mandates per-sentence citation. |
...
of the urgency level and are surfaced to callers (`routers/medicine.py` shows
an explicit "About changing a dose" warning banner when
`dosage_change_request` is flagged). They do not currently alter the chat
...
- Classifier is English-pattern-only; Hindi/Gujarati emergency phrases in the
  user's own words would not be recognized (the UI's language selector
  affects *response* language, not input classification).
